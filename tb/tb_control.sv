...
module tb_control;
    ...
    initial begin
        ...
        ck1("分支: flush_ex2mem", flush_ex2mem, 1'b1);
        ...
        $display("\n-- 异常 --");
        idle(); ex_illegal=1; #1;
        // 异常只 flush 流水线；当前设计无异常处理（无 mcause/mepc），
        // 不应 redirect 到未定义地址
        ck1("illegal: redirect_en=0", redirect_en, 1'b0);
        ...
        ck1("illegal: flush_ex2mem", flush_ex2mem, 1'b1);
        idle(); ex_ecall=1; #1;
